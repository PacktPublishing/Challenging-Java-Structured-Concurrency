...
public class Main {
    ...
    private static final String username = "fabio88";                                  // login user
    private static final String clientLocation = "123 East 55st Street";               // location shared by the client
    ...
    private static final boolean validPublicHighwayEcsCoupon = true;                   // public highway ECS coupon
    private static final boolean validPublicDriveinEcsCoupon = true;                   // public drive-in ECS coupon
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        try {
            Charger ecsCharger = ScopedValue.where(USERNAME, username)
    ...
    public static Charger findECS() throws InterruptedException {
        ...
        if (CLIENT_LOCATION.get() != null) {
            try (var scope = StructuredTaskScope.open(new ChargeFinderScope(),
                    cf -> cf.withName("ECSScope")
                            .withThreadFactory(Thread.ofVirtual().name("ecs-", 0).factory()))) {
    ...
    private static PrivateECS findPrivateECS() throws InterruptedException {
        ...
        try (var scope = StructuredTaskScope.open(Joiner.<PrivateECS>awaitAll(),
                cf -> cf.withName("PrivateECSScope")
                        .withThreadFactory(Thread.ofVirtual().name("privateECS-", 0).factory()))) {
    ...
    private static PublicECS findPublicECS() throws InterruptedException {
        ...
        try (var scope = StructuredTaskScope.open(new PublicChargeFinderScope(),
                cf -> cf.withName("PublicECSScope")
                        .withThreadFactory(Thread.ofVirtual().name("publicECS-", 0).factory()))) {
