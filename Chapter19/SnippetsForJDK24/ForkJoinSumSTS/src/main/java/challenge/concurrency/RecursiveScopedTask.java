...
record RecursiveScopedTask(List<Integer> intlist, int start, int end) {
    ...
    protected Void compute(List<Integer> intlist, StructuredTaskScope<Object> scope) {
        ...
        scope.fork(() -> taskLeft.compute(intlistLeft, scope));

        RecursiveScopedTask taskRight = new RecursiveScopedTask(intlistRight, 0, intlistRight.size());
        scope.fork(() -> taskRight.compute(intlistRight, scope));
