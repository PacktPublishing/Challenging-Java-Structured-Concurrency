...
public final class Conveyor {
    ...
    private static final TransferQueue<String> batteryQueue = new LinkedTransferQueue<>();

    private static final int MAXIMUM_CHARGING_TIME = 2 * 1000;
    private static final int MAXIMUM_DRAINING_TIME = 2 * 1000;

    private static final int TRANSFER_TIMEOUT = MAXIMUM_DRAINING_TIME + 1000;
        ...
        public void run() {
            while (chargerOn) {
                try {
                    ...
                    boolean success = batteryQueue.tryTransfer(
                            battery, TRANSFER_TIMEOUT, TimeUnit.MILLISECONDS);
            ...
            while (drainerOn) {
                try {
                    String battery = batteryQueue.poll(
                            MAXIMUM_CHARGING_TIME, TimeUnit.MILLISECONDS);
