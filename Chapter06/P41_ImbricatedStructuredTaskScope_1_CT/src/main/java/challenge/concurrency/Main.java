...
public class Main {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        Subtask fastSubtask = null;
        Subtask additionalSubtask = null;
        try (var fastScope = StructuredTaskScope.open()) {

            fastSubtask = fastScope.fork(() -> fastTask());

            additionalSubtask = fastScope.fork(() -> {
            ...
            State fastSubtaskState = fastSubtask.state();
            State additionalSubtaskState = additionalSubtask.state();
            
            logger.info(() -> "Fast subtask state:" + fastSubtaskState);
            logger.info(() -> "Additional subtask state:" + additionalSubtaskState);
        }
    }
}