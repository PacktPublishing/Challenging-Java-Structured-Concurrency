...
public final class Conveyor {
    ...
    private static final int MAXIMUM_BATTERY_QUEUE = 10; // bounded queue
    ...
    private static final AtomicInteger rcsTerminals = new AtomicInteger(0);
    ...
    private static void rcs() {
        ...
        while (chargerOn && drainerOn) {
            ...
            if (batteryQueue.isEmpty()) {
                if (MAXIMUM_ROBOTIC_TERMINALS == rcsTerminals.incrementAndGet()) {
                    fillBatteryQueue(); // after all terminals have been collected, we can fill up the queue
                } else {
                    logger.info(() -> "Terminal " + Thread.currentThread() + " waits for queue to be filled up ...");
                    while (rcsTerminals.get() != 0) {} // until the queue is filled up the terminals hang on in this loop
                }
            } else {
    ...
    private static synchronized void fillBatteryQueue() { // we ensure that only one thread can fill up the queue

        for (int i = 0; i < MAXIMUM_BATTERY_QUEUE; i++) {
            batteryQueue.offer("battery-" + rnd.nextInt(1000));
        }

        logger.info(() -> "A new set of batteries was added to the queue by: " + Thread.currentThread());
        rcsTerminals.set(0); // the queue is full, so the terminals can continue their work
    }
