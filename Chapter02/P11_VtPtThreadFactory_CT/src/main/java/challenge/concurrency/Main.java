...
public class Main {
    ...
    private static class AdminThreadFactory implements VtPtThreadFactory {
        ...
        private static final Thread.Builder.OfVirtual ofv = Thread.ofVirtual();
        private static final Thread.Builder.OfPlatform ofp = Thread.ofPlatform();

        @Override
        public Thread newThread(Runnable vt, boolean virtual) {
            return virtual ? ofv.unstarted(vt) : ofp.unstarted(vt);
        }
